(function(){
  ...
  // --- Estado ---
  let mode='';                // 'ids' | 'auto'
  let bucketA=[], bucketB=[]; // hojas con FILL a pintar
  ...
  function normColor(c){ if(!c) return null; const ctx=document.createElement('canvas').getContext('2d'); ctx.fillStyle=c; return ctx.fillStyle; }
  ...
  function buildBucketsById(root){
    ...
    if(ids['stripe1'] && ids['stripe2']){
      ...
      dbg.innerHTML = `✅ SVG cargado (modo <b>ids</b>)<br>stripe1 (fill): <b>${bucketA.length}</b><br>stripe2 (fill): <b>${bucketB.length}</b>`;
  ...
  // --- Pintado (SOLO fill; NO tocamos stroke) ---
  function paint(){
    ...
    bucketA.forEach(o=>{ o.set('fill', patA); o.dirty=true; });
    bucketB.forEach(o=>{ o.set('fill', patB); o.dirty=true; });
